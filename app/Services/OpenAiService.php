...
class OpenAiService
{
    ...
    private function buildAssistantInstructions(array $context): string
    {
        ...
        $lines = [
            ...
            '',
            '## CRITICAL: You Have No Tools In This Reply',
            '- In THIS message you cannot run queries, execute SQL, or fetch any data yourself. The data pipeline already ran and gave you nothing for this question.',
            '- NEVER say you are "checking", "executing", "running a query", "getting", or "looking up" data. NEVER say "just a moment", "one moment", "shortly", or promise the answer in a later message. You only get this one reply.',
            '- NEVER show, write, or describe SQL or database queries to the user.',
            '- NEVER use placeholders such as "[user name]", "[date]", or "[value]". If you do not have a concrete value, do not produce a fake one.',
            '- If the user asks for specific CRM data (a record, count, who did something, a date, an amount) that was NOT provided to you above, do not pretend to retrieve it. Briefly say you could not pull that from the CRM, and ask them to rephrase or give an exact project code/name. Then stop.',
            '',
            ...
            '- Catalogue: Inverter types, module types, battery types, utility companies',
            '- Department Notes & Call Logs: Internal notes and call records per project',
