{{--
    The Sales Partners screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'sales-partners', 'id' => $id]))
        : route('sales.partner.types', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($partner) ? route('sales.partner.update',$partner->id) :  route('sales.partner.store') }}" enctype="multipart/form-data">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="sales-partners" />
            @endif
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <a class="action-link" data-toggle="tooltip" title="Edit" href="{{ $screenUrl($partnerList->id)}}">
