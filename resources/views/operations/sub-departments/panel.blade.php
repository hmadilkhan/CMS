{{--
    The Sub Departments screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'sub-departments', 'id' => $id]))
        : route('sub.departments.list', array_filter(['id' => $id]));
@endphp
@if (session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        <form class="operation-form" method="POST" action="{{ !empty($subDepartment) ? route('sub.department.update', $subDepartment->id) : route('sub.department.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="sub-departments" />
            @endif
