{{--
    The Inverter Types screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'inverter-types', 'id' => $id]))
        : route('view-inverter-type', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($inverterType) ? route('inverter.type.update',$inverterType->id) :  route('inverter.type.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="inverter-types" />
            @endif
