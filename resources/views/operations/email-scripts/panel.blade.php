{{--
    The Email Scripts screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = function ($params = []) use ($console) {
        $params = array_filter(is_array($params) ? $params : ['id' => $params]);

        return $console
            ? route('operations.console', ['section' => 'email-scripts'] + $params)
            : route('email.scripts.list', $params);
    };
@endphp
    ...
    <div class="card operation-card">
        ...
        <div class="card-body">
            <form class="operation-form" method="POST"
                ...
                @csrf
                @if ($console)
                <input type="hidden" name="ops_section" value="email-scripts" />
                @endif
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <div class="operation-actions">
                            ...
                            <a href="{{ $screenUrl() }}" class="btn btn-outline-secondary"><i
                    ...
                    @foreach ($notificationTemplates as $templateKey => $template)
                        <tr>
                            ...
                            <td class="text-center text-nowrap">
                                <a class="action-link" data-toggle="tooltip" title="Edit"
                                    href="{{ $screenUrl(['template' => $templateKey]) }}">
                                    <i class="icofont-pencil text-warning"></i></a>
