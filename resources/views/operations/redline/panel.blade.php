{{--
    The Inverter Base Cost screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'inverter-base-cost', 'id' => $id]))
        : route('view-redline-cost', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($redline) ? route('redlinecost.update',$redline->id) :  route('redlinecost.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="inverter-base-cost" />
            @endif
