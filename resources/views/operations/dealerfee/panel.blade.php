{{--
    The Dealer Fee screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'dealer-fee', 'id' => $id]))
        : route('view-dealer-fee', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($loan) ? route('dealerfee.update',$loan->id) :  route('dealerfee.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="dealer-fee" />
            @endif
