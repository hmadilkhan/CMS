{{--
    The Adders screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'adders', 'id' => $id]))
        : route('view-adders', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($adder) ? route('adders.update',$adder->id) :  route('adder.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="adders" />
            @endif
