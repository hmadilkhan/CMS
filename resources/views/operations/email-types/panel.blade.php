{{--
    The Email Types screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'email-types', 'id' => $id]))
        : route('email.types.list', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        <form class="operation-form" method="POST" action="{{ !empty($emailType) ? route('email.types.update', $emailType->id) : route('email.types.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="email-types" />
            @endif
