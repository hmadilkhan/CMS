{{--
    The Adder Types screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'adder-types', 'id' => $id]))
        : route('view.adder.types', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($adder) ? route('adder.type.update',$adder->id) :  route('adder.type.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="adder-types" />
            @endif
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <a class="action-link" data-toggle="tooltip" title="Edit" href="{{ $screenUrl($adderList->id) }}">
