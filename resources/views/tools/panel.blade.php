{{--
    The Tools screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'tools', 'id' => $id]))
        : route('tools.manage', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($tool) ? route('tools.update',$tool->id) :  route('tools.store') }}" enctype="multipart/form-data">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="tools" />
            @endif
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <a class="action-link" data-toggle="tooltip" title="Edit" href="{{ $screenUrl($tool->id) }}">
