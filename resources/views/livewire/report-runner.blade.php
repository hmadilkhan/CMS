                                ...
                                <select wire:model.live="selectedReportId" class="form-select" id="reportSelect">
                                    ...
                                    @foreach($this->userReports as $report)
                                        <option value="{{ $report->id }}">
                                            {{ $report->name }} ({{ ucfirst($report->report_type) }}) - {{ $report->created_at->format('M d, Y') }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div class="col-12">
                                    <div class="alert alert-info">
                                        <h6 class="alert-heading mb-2">
                                            <i class="icofont-info-circle me-2"></i>Report Details: {{ $selectedReport->name }}
                                        </h6>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <strong>Type:</strong> {{ ucfirst($selectedReport->report_type) }}<br>
                                                <strong>Fields:</strong> {{ count($selectedReport->selected_fields) }} selected<br>
                                                <strong>Created:</strong> {{ $selectedReport->created_at->format('M d, Y \a\t g:i A') }}
                                            </div>
                                            <div class="col-md-6">
                                                <strong>Filters:</strong> {{ count($selectedReport->filters ?? []) }} filters<br>
                                                <strong>Calculated Fields:</strong> {{ count($selectedReport->calculated_fields ?? []) }} fields
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0">
                                <i class="icofont-table me-2"></i>{{ $selectedReport->name }} - Results ({{ count($reportData) }} records)
                            </h5>
                            <div class="btn-group">
                        ...
                        <div class="card-body">
                            ...
                            @else
                                <div class="text-center py-5">
                                    ...
                                    <p class="text-muted">The report executed successfully but returned no results. Try adjusting your filter values.</p>
                                </div>
                            @endif
                        </div>
                    </div>
