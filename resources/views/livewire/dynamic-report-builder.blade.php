                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <input type="text" wire:model.live="reportName" class="form-control"
                                    id="reportName" />
                                @error('reportName')
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                ...
                                <div class="row">
                                    @php
                                        $fieldGroups = [
                                            'Customer Fields' => collect($this->availableFields)->filter(
                                                fn($name, $field) => str_starts_with($field, 'customers.'),
                                            ),
                                            'Project Fields' => collect($this->availableFields)->filter(
                                                fn($name, $field) => str_starts_with($field, 'projects.'),
                                            ),
                                            'Sales Partner Fields' => collect($this->availableFields)->filter(
                                                fn($name, $field) => str_starts_with($field, 'sales_partners.'),
                                            ),
                                            'Department Fields' => collect($this->availableFields)->filter(
                                                fn($name, $field) => str_starts_with($field, 'departments.') ||
                                                    str_starts_with($field, 'sub_departments.'),
                                            ),
                                            'Type Fields' => collect($this->availableFields)->filter(
                                                fn($name, $field) => str_starts_with($field, 'module_types.') ||
                                                    str_starts_with($field, 'inverter_types.'),
                                            ),
                                            'Finance Fields' => collect($this->availableFields)->filter(
                                                fn($name, $field) => str_starts_with($field, 'customer_finances.'),
                                            ),
                                        ];
                                    @endphp
                                                        ...
                                                        @foreach ($fields as $field => $name)
                                                            <div class="form-check form-check-sm mb-1">
                                                                ...
                                                                <label class="form-check-label small"
                                                                    for="field_{{ str_replace('.', '_', $field) }}">
                                                                    {{ $name }}
                                                                </label>
                                                            </div>
                                                        @endforeach
                                    ...
                                    <div class="mt-3">
                                        <details class="border p-2 rounded">
                                            ...
                                            <small class="text-muted">
                                                <strong>Selected Fields Count:</strong>
                                                {{ $this->selectedFieldsCount }}<br>
                            ...
                            <div class="col-12">
                                ...
                                <div class="row g-2 align-items-end mb-2">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="number" wire:model="builderValue2" class="form-control mt-1"
                                    ...
                                    <div class="col-md-2 d-flex gap-1">
                                        ...
                                        <button type="button" wire:click="removeLastCalcBuilder"
                                            class="btn btn-outline-danger w-100">Undo</button>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-md-10">
                                        ...
                                        <div class="form-control bg-light">
                                            {{ $calcExpressionPreview ?: 'Build your expression...' }}</div>
                                        <small class="text-muted">You can chain multiple operations (e.g., A + B - C *
                                            D). Use "Undo" to remove the last operation.</small>
                                    </div>
                                    <div class="col-md-2 d-flex align-items-end">
                                        <button type="button" wire:click="useCalcBuilder"
                                            class="btn btn-success w-100">Use Expression</button>
                                    </div>
                                </div>
                                ...
                                <div class="row g-3 align-items-end">
                                    ...
                                    <div class="col-md-7">
                                        ...
                                        <small class="text-muted">Use the builder above or type manually (use
                                            {field.name} format for field references)</small>
                                    </div>
                                    <div class="col-md-2">
                                        <button type="button" wire:click="addCalculatedField"
                                ...
                                @if (!empty($calculatedFields))
                                    <div class="mt-3">
                                        <h6 class="fw-bold mb-2">Calculated Fields ({{ count($calculatedFields) }})
                                        </h6>
                                        <div class="d-flex flex-wrap gap-2">
                        ...
                        <div class="row">
                            <div class="col-12  mt-2">
                                <div class="d-flex justify-content-center">
                                    @if ($isEditing)
                                        <button type="button" wire:click="updateReport" class="btn btn-success">
                                            <i class="icofont-save me-1"></i>Update Report
                                        </button>
                                        <button type="button" wire:click="cancelEdit"
                                            class="btn btn-secondary ms-2">
                                            <i class="icofont-close me-1"></i>Cancel Edit
                                        </button>
                                    @else
                                        <button type="button" wire:click="saveReport" class="btn btn-success">
