{{--
    The Office Cost screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'office-costs', 'id' => $id]))
        : route('office-costs.index', array_filter(['id' => $id]));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ route('office-costs.store') }}">
            @csrf
            @if ($console)
            <input type="hidden" name="ops_section" value="office-costs" />
            @endif
