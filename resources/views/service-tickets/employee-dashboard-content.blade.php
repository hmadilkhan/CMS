<style>
    ...
    .employee-ticket-modal-header .modal-title,
    .employee-ticket-modal-header i {
        color: #050505 !important;
    }
    .project-link {
        color: #1d4ed8;
...
<div class="row mb-4">
    <div class="col-md-4">
        <div class="premium-widget">
            ...
            <h2 class="widget-value">{{ $serviceTickets->count() }}</h2>
            <p class="widget-label mb-0">My Tickets</p>
        </div>
    </div>
    <div class="col-md-4">
        <div class="premium-widget widget-pending">
            <div class="widget-icon"><i class="icofont-clock-time"></i></div>
            <h2 class="widget-value">{{ $serviceTickets->where('status', 'Pending')->count() }}</h2>
    ...
    <div class="col-md-4">
        <div class="premium-widget widget-high">
            ...
            <h2 class="widget-value">{{ $serviceTickets->where('priority', 'High')->count() }}</h2>
...
<div class="modal fade" id="employeeTicketModal" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
            <div class="modal-header employee-ticket-modal-header">
                <h5 class="modal-title fw-bold"><i class="icofont-ticket me-2"></i>Ticket Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body" style="padding: 2rem;">
                <div id="employeeTicketDetails"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
function viewEmployeeTicket(ticketId) {
    $.ajax({
        url: '/service-tickets/' + ticketId + '/details',
        method: 'GET',
        success: function(response) {
            $('#employeeTicketDetails').html(response);
            $('#employeeTicketModal').modal('show');
        },
