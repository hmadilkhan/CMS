        ...
        if ($(this).val() != "") {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    let amount = price;
                    if (unitId == 3) {
                        let moduleQty = parseFloat("{{ (float) ($project->customer->module_value ?? 0) }}") || 0;
                        console.log("{{ (float) ($project->customer->module_value ?? 0) }}");
                        
                        amount = price * moduleQty;
                    } else if (unitId == 5) {
                        let panelQty = parseFloat("{{ (float) ($project->customer->panel_qty ?? 0) }}") || 0;
                        console.log("{{ (float) ($project->customer->panel_qty ?? 0) }}");
