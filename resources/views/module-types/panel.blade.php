{{--
    The Module Types screen's body - see operations/departments/panel.blade.php
    for what `$console` means here.
--}}
@php
    $console = $console ?? false;
    $screenUrl = fn ($id = null) => $console
        ? route('operations.console', array_filter(['section' => 'module-types', 'id' => $id]))
        : ($id ? route('module-types.edit', $id) : route('module-types.index'));
@endphp
@if(session('success'))
...
<div class="card operation-card">
    ...
    <div class="card-body">
        ...
        <form class="operation-form" method="POST" action="{{ !empty($type) ? route('module-types.update',$type->id) :  route('module-types.store') }}">
            ...
            @if ($console)
            <input type="hidden" name="ops_section" value="module-types" />
            @endif
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <a class="action-link" data-toggle="tooltip" title="Edit" href="{{ $screenUrl($type->id) }}">
